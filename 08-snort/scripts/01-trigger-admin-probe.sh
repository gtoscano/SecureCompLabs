...
# Purpose:
# - Generate HTTP traffic that triggers Snort rule sid:1000002 ("/admin" probe).
# - Capture that traffic into a host file: ./pcaps/01-admin-probe.pcap.
...
PCAP_OUT="$PCAP_DIR/01-admin-probe.pcap"
...
curl -sS "http://127.0.0.1/admin" >/dev/null || true
sleep 1
