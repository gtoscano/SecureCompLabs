...
# Purpose:
# - Generate HTTP traffic that triggers Snort rule sid:1000003 (command separators ;, ||, &&).
# - Capture that traffic into a host file: ./pcaps/03-cmd-injection.pcap.
...
PCAP_OUT="$PCAP_DIR/03-cmd-injection.pcap"
...
curl -sS -X POST "http://127.0.0.1/" -d "cmd=ls&&whoami" >/dev/null || true
sleep 1
